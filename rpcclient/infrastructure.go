...
// Client represents a Navcoin RPC client which allows easy access to the
// various RPC methods available on a Navcoin RPC server.  Each of the wrapper
...
func (c *Client) trackRegisteredNtfns(cmd interface{}) {
	...
	case *navjson.NotifyBlocksCmd:
		c.ntfnState.notifyBlocks = true

	case *navjson.NotifyNewTransactionsCmd:
...
type (
	...
	rawResponse struct {
		...
		Error  *navjson.RPCError `json:"error"`
	}
)
...
// returning an unmarshaled navjson.RPCError (or an unmarshaling error) if so.
...
func (c *Client) reregisterNtfns() error {
	...
	if nslen > 0 {
		outpoints := make([]navjson.OutPoint, 0, nslen)
...
func (c *Client) sendCmd(cmd interface{}) chan *response {
	...
	method, err := navjson.CmdMethod(cmd)
	if err != nil {
	...
	// Marshal the command.
	id := c.NextID()
	marshalledJSON, err := navjson.MarshalCmd(id, cmd)
	if err != nil {
		return newFutureError(err)
	}
