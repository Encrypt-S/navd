...
// newAddressPubKey returns a new navutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) navutil.Address {
	addr, err := navutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new navutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) navutil.Address {
	addr, err := navutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new navutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) navutil.Address {
	addr, err := navutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the navutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the navutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := navutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := navutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := navutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := navutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := navutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*navutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*navutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*navutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*navutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*navutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
...
var scriptClassTests = []struct {
	...
	// codeseparator parts have been elided. (navcoin core's checks for
