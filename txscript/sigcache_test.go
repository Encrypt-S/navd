...
// test data.
func genRandomSig() (*chainhash.Hash, *navec.Signature, *navec.PublicKey, error) {
	privKey, err := navec.NewPrivateKey(navec.S256())
	if err != nil {
...
func TestSigCacheAddExists(t *testing.T) {
	...
	sig1Copy, _ := navec.ParseSignature(sig1.Serialize(), navec.S256())
	key1Copy, _ := navec.ParsePubKey(key1.SerializeCompressed(), navec.S256())
...
func TestSigCacheAddEvictEntry(t *testing.T) {
	...
	for i := uint(0); i < sigCacheSize; i++ {
		...
		sigCopy, _ := navec.ParseSignature(sig.Serialize(), navec.S256())
		keyCopy, _ := navec.ParsePubKey(key.SerializeCompressed(), navec.S256())
	...
	sigNewCopy, _ := navec.ParseSignature(sigNew.Serialize(), navec.S256())
	keyNewCopy, _ := navec.ParsePubKey(keyNew.SerializeCompressed(), navec.S256())
