...
func Example_signMessage() {
	...
	privKey, pubKey := navec.PrivKeyFromBytes(navec.S256(), pkBytes)
...
func Example_verifySignature() {
	...
	pubKey, err := navec.ParsePubKey(pubKeyBytes, navec.S256())
	if err != nil {
		fmt.Println(err)
		return
	}
...
func Example_encryptMessage() {
	...
	message := "test message"
	ciphertext, err := navec.Encrypt(pubKey, []byte(message))
	if err != nil {
		fmt.Println(err)
		return
	}
	...
	privKey, _ := navec.PrivKeyFromBytes(navec.S256(), pkBytes)

	// Try decrypting and verify if it's the same message.
	plaintext, err := navec.Decrypt(privKey, ciphertext)
	if err != nil {
		fmt.Println(err)
		return
	}
