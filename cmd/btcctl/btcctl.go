...
func main() {
	...
	method := args[0]
	usageFlags, err := navjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := navjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// navjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(navjson.Error); ok {
		...
		// The error is not a navjson.Error and this really should not
	...
	marshalledJSON, err := navjson.MarshalCmd(1, cmd)
	if err != nil {
