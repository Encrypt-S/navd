...
const (
	// svcName is the name of navd service.
	svcName = "navdsvc"
	...
	svcDesc = "Downloads and stays synchronized with the navcoin block " +
...
// logServiceStartOfDay logs information about navd when the main server has
...
// navdService houses the main service handler which handles all service
// updates and launching navdMain.
type navdService struct{}
...
// long-running navdMain (which is the real meat of navd), handles service
// change requests, and notifies the service control manager of changes.
func (s *navdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start navdMain in a separate goroutine so the service can start
...
// installService attempts to install the navd service.  Typically this should
...
// removeService attempts to uninstall the navd service.  Typically this should
...
// startService attempts to start the navd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &navdService{})
	if err != nil {
