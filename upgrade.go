...
// oldBtcdHomeDir returns the OS specific home directory navd used prior to
// version 0.3.3.  This has since been replaced with navutil.AppDataDir, but
// this function is still provided for the automatic upgrade path.
func oldBtcdHomeDir() string {
	// Search for Windows APPDATA first.  This won't exist on POSIX OSes.
	appData := os.Getenv("APPDATA")
	if appData != "" {
		return filepath.Join(appData, "navd")
	}

	// Fall back to standard HOME directory that works for most POSIX OSes.
	home := os.Getenv("HOME")
	if home != "" {
		return filepath.Join(home, ".navd")
	}
...
// location prior to navd version 0.2.0 and uses heuristics to ascertain the old
...
// upgradeDBPaths moves the databases from their locations prior to navd
...
func upgradeDBPaths() error {
	...
	upgradeDBPathNet(filepath.Join(oldDbRoot, "navd.db"), "mainnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "navd_testnet.db"), "testnet")
	upgradeDBPathNet(filepath.Join(oldDbRoot, "navd_regtest.db"), "regtest")

	// Remove the old db directory.
	return os.RemoveAll(oldDbRoot)
}

// upgradeDataPaths moves the application data from its location prior to navd
...
func upgradeDataPaths() error {
	...
	if fileExists(oldHomePath) && !fileExists(newHomePath) {
		// Create the new path.
		navdLog.Infof("Migrating application home path from '%s' to '%s'",
		...
		// Move old navd.conf into new location if needed.
		...
		if ohpEmpty {
			...
			navdLog.Warnf("Not removing '%s' since it contains files "+
...
// doUpgrades performs upgrades to navd as new versions require it.
