...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: navutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := navutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*navutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, navutil.NewTx(tx))
	}
...
func additionalCoinbase(amount navutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee navutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee navutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee navutil.Amount) *wire.MsgTx {
