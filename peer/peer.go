...
type MessageListeners struct {
	// OnGetAddr is invoked when a peer receives a getaddr navcoin message.
	OnGetAddr func(p *Peer, msg *wire.MsgGetAddr)

	// OnAddr is invoked when a peer receives an addr navcoin message.
	OnAddr func(p *Peer, msg *wire.MsgAddr)

	// OnPing is invoked when a peer receives a ping navcoin message.
	OnPing func(p *Peer, msg *wire.MsgPing)

	// OnPong is invoked when a peer receives a pong navcoin message.
	OnPong func(p *Peer, msg *wire.MsgPong)

	// OnAlert is invoked when a peer receives an alert navcoin message.
	OnAlert func(p *Peer, msg *wire.MsgAlert)

	// OnMemPool is invoked when a peer receives a mempool navcoin message.
	OnMemPool func(p *Peer, msg *wire.MsgMemPool)

	// OnTx is invoked when a peer receives a tx navcoin message.
	OnTx func(p *Peer, msg *wire.MsgTx)

	// OnBlock is invoked when a peer receives a block navcoin message.
	OnBlock func(p *Peer, msg *wire.MsgBlock, buf []byte)

	// OnCFilter is invoked when a peer receives a cfilter navcoin message.
	OnCFilter func(p *Peer, msg *wire.MsgCFilter)

	// OnCFHeaders is invoked when a peer receives a cfheaders navcoin
	// message.
	OnCFHeaders func(p *Peer, msg *wire.MsgCFHeaders)

	// OnCFTypes is invoked when a peer receives a cftypes navcoin message.
	OnCFTypes func(p *Peer, msg *wire.MsgCFTypes)

	// OnInv is invoked when a peer receives an inv navcoin message.
	OnInv func(p *Peer, msg *wire.MsgInv)

	// OnHeaders is invoked when a peer receives a headers navcoin message.
	OnHeaders func(p *Peer, msg *wire.MsgHeaders)

	// OnNotFound is invoked when a peer receives a notfound navcoin
	// message.
	OnNotFound func(p *Peer, msg *wire.MsgNotFound)

	// OnGetData is invoked when a peer receives a getdata navcoin message.
	OnGetData func(p *Peer, msg *wire.MsgGetData)

	// OnGetBlocks is invoked when a peer receives a getblocks navcoin
	// message.
	OnGetBlocks func(p *Peer, msg *wire.MsgGetBlocks)

	// OnGetHeaders is invoked when a peer receives a getheaders navcoin
	// message.
	OnGetHeaders func(p *Peer, msg *wire.MsgGetHeaders)

	// OnGetCFilter is invoked when a peer receives a getcfilter navcoin
	// message.
	...
	// OnGetCFTypes is invoked when a peer receives a getcftypes navcoin
	// message.
	OnGetCFTypes func(p *Peer, msg *wire.MsgGetCFTypes)

	// OnFeeFilter is invoked when a peer receives a feefilter navcoin message.
	OnFeeFilter func(p *Peer, msg *wire.MsgFeeFilter)

	// OnFilterAdd is invoked when a peer receives a filteradd navcoin message.
	OnFilterAdd func(p *Peer, msg *wire.MsgFilterAdd)

	// OnFilterClear is invoked when a peer receives a filterclear navcoin
	// message.
	OnFilterClear func(p *Peer, msg *wire.MsgFilterClear)

	// OnFilterLoad is invoked when a peer receives a filterload navcoin
	// message.
	OnFilterLoad func(p *Peer, msg *wire.MsgFilterLoad)

	// OnMerkleBlock  is invoked when a peer receives a merkleblock navcoin
	// message.
	OnMerkleBlock func(p *Peer, msg *wire.MsgMerkleBlock)

	// OnVersion is invoked when a peer receives a version navcoin message.
	OnVersion func(p *Peer, msg *wire.MsgVersion)

	// OnVerAck is invoked when a peer receives a verack navcoin message.
	OnVerAck func(p *Peer, msg *wire.MsgVerAck)

	// OnReject is invoked when a peer receives a reject navcoin message.
	OnReject func(p *Peer, msg *wire.MsgReject)

	// OnSendHeaders is invoked when a peer receives a sendheaders navcoin
	// message.
	OnSendHeaders func(p *Peer, msg *wire.MsgSendHeaders)

	// OnRead is invoked when a peer receives a navcoin message.  It
	...
	// OnWrite is invoked when we write a navcoin message to a peer.  It
...
// net.Addr interface and create a navcoin NetAddress structure using that
// information.
...
// Peer provides a basic concurrent safe navcoin peer for handling navcoin
...
// callbacks for handling navcoin protocol messages.
//
...
func (p *Peer) localVersionMsg() (*wire.MsgVersion, error) {
	...
	// XXX: navcoind appears to always enable the full node services flag
	// of the remote peer netaddress field in the version message regardless
	// of whether it knows it supports it or not.  Also, navcoind sets
...
// handleRemoteVersionMsg is invoked when a version navcoin message is received
...
// handlePingMsg is invoked when a peer receives a ping navcoin message.  For
...
// handlePongMsg is invoked when a peer receives a pong navcoin message.  It
...
// readMessage reads the next navcoin message from the peer with logging.
...
// writeMessage sends a navcoin message to the peer with logging.
...
// QueueMessage adds the passed navcoin message to the peer send queue.
//
// This function is safe for concurrent access.
...
// QueueMessageWithEncoding adds the passed navcoin message to the peer send
...
// newPeerBase returns a new base navcoin peer based on the inbound flag.  This
...
// NewInboundPeer returns a new inbound navcoin peer. Use Start to begin
// processing incoming and outgoing messages.
func NewInboundPeer(cfg *Config) *Peer {
	return newPeerBase(cfg, true)
}

// NewOutboundPeer returns a new outbound navcoin peer.
