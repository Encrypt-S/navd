...
func (eft *estimateFeeTester) testTx(fee navutil.Amount) *TxDesc {
	eft.version++
	return &TxDesc{
		TxDesc: mining.TxDesc{
			Tx: navutil.NewTx(&wire.MsgTx{
				Version: eft.version,
			}),
			Height: eft.height,
			Fee:    int64(fee),
...
func (eft *estimateFeeTester) newBlock(txs []*wire.MsgTx) {
	...
	block := navutil.NewBlock(&wire.MsgBlock{
		Transactions: txs,
	})
...
func (eft *estimateFeeTester) round(txHistory [][]*TxDesc,
	...
	for i := uint32(0); i < txPerRound; i++ {
		newTx := eft.testTx(navutil.Amount(rand.Intn(1000000)))
