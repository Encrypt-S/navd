...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if navjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *navjson.RPCError {
				return navjson.NewRPCError(navjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := navjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := navjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := navjson.Error{ErrorCode: navjson.ErrInvalidType}
	_, err = navjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(navjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = navjson.MarshalResponse(1, make(chan int), nil)
