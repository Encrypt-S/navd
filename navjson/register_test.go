...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   navjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{navjson.UFWalletOnly, "UFWalletOnly"},
		{navjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{navjson.UFNotification, "UFNotification"},
		{navjson.UFWalletOnly | navjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{navjson.UFWalletOnly | navjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := navjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: navjson.Error{ErrorCode: navjson.ErrDuplicateMethod},
		},
		{
			...
			flags: navjson.TstHighestUsageFlagBit,
			err:   navjson.Error{ErrorCode: navjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: navjson.Error{ErrorCode: navjson.ErrEmbeddedType},
		},
		{
			...
			err: navjson.Error{ErrorCode: navjson.ErrUnexportedField},
		},
		{
			...
			err: navjson.Error{ErrorCode: navjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: navjson.Error{ErrorCode: navjson.ErrNonOptionalField},
		},
		{
			...
			err: navjson.Error{ErrorCode: navjson.ErrNonOptionalDefault},
		},
		{
			...
			err: navjson.Error{ErrorCode: navjson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := navjson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	navjson.MustRegisterCmd("panicme", 0, 0)
}
...
func TestRegisteredCmdMethods(t *testing.T) {
	...
	methods := navjson.RegisteredCmdMethods()
	if len(methods) == 0 {
