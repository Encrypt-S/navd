...
differences between navd and bitcoind as far as how RPCs are serviced:
* Unlike bitcoind that has the wallet and chain intermingled in the same process
  ...
  [here](https://blog.conformal.com/navd-not-your-moms-bitcoin-daemon/) for
...
The following is an overview of the RPC methods which are implemented by navd, but not the original bitcoind client. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get bitcoin network navd is running on.|None|
...
|Parameters|None|
|Description|Get bitcoin network navd is running on.|
|Returns|numeric|
