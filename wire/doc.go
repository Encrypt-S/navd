...
/*
Package wire implements the navcoin wire protocol.

For the complete details of the navcoin protocol, see the official wiki entry
...
supported navcoin messages to and from the wire.  This package does not deal
...
which navcoin network it is a part of, its type, how big it is, and a checksum
...
of the currently supported navcoin messages are provided.  For these supported
messages, all of the details of marshalling and unmarshalling to and from the
wire using navcoin encoding are handled so the caller doesn't have to concern
themselves with the specifics.

Message Interaction

The following provides a quick summary of how the navcoin messages are intended
to interact with one another.  As stated above, these interactions are not
directly handled by this package.  For more in-depth details about the
appropriate interactions, see the official navcoin protocol wiki entry at
...
and write navcoin messages.  The following sections provide a quick overview of
...
As discussed in the navcoin message overview section, this package reads
and writes navcoin messages using a generic interface named Message.  In
...
In order to unmarshall navcoin messages from the wire, use the ReadMessage
...
In order to marshall navcoin messages to the wire, use the WriteMessage
function.  It accepts any io.Writer, but typically this will be a net.Conn to
a remote node running a navcoin peer.  Example syntax to request addresses
...
This package includes spec changes outlined by the following BIPs:
	...
	BIP0014 (https://github.com/navcoin/bips/blob/master/bip-0014.mediawiki)
	BIP0031 (https://github.com/navcoin/bips/blob/master/bip-0031.mediawiki)
	BIP0035 (https://github.com/navcoin/bips/blob/master/bip-0035.mediawiki)
	BIP0037 (https://github.com/navcoin/bips/blob/master/bip-0037.mediawiki)
	BIP0111	(https://github.com/navcoin/bips/blob/master/bip-0111.mediawiki)
	BIP0130 (https://github.com/navcoin/bips/blob/master/bip-0130.mediawiki)
	BIP0133 (https://github.com/navcoin/bips/blob/master/bip-0133.mediawiki)
*/
package wire
