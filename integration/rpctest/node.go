...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	navdPath, err := navdExecutablePath()
	if err != nil {
		navdPath = "navd"
	}
...
// arguments returns an array of arguments that be used to launch the navd
// process.
...
// command returns the exec.Cmd which will be used to start the navd process.
...
// to the navd process that is launched via Start().
...
// as the base for the log and data directories for navd.
...
// start creates a new navd process, and writes its pid in a file reserved for
...
// stop interrupts the running navd process process, and waits until it exits
...
// shutdown terminates the running navd process, and cleans up all
