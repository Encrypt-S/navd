...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt navutil.Amount) *chainhash.Hash {
	...
	// input.
	txid := genSpend(navutil.Amount(5 * navutil.SatoshiPerBitcoin))
	...
	txid = genSpend(navutil.Amount(500 * navutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(navutil.SatoshiPerBitcoin, pkScript)
	...
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, navutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	expectedBalance := navutil.Amount(250 * navutil.SatoshiPerBitcoin)
	...
	// chain.
	expectedBalance = navutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := navutil.Amount(50 * navutil.SatoshiPerBitcoin)
...
func TestHarness(t *testing.T) {
	...
	// outputs.
	expectedBalance := navutil.Amount(numMatureOutputs * 50 * navutil.SatoshiPerBitcoin)
