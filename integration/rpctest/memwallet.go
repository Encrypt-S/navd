...
import (
	...
	"github.com/encrypt-s/navutil"
	"github.com/encrypt-s/navutil/hdkeychain"
)
...
type memWallet struct {
	...
	addrs map[uint32]navutil.Address
...
func newMemWallet(net *chaincfg.Params, harnessID uint32) (*memWallet, error) {
	...
	// newly generated navcoin we can spend.
	addrs := make(map[uint32]navutil.Address)
...
// SetRPCClient saves the passed rpc connection to navd as the wallet's
...
func (m *memWallet) IngestBlock(height int32, header *wire.BlockHeader, filteredTxns []*navutil.Tx) {
...
func (m *memWallet) evalOutputs(outputs []*wire.TxOut, txHash *chainhash.Hash,
	...
	for i, output := range outputs {
		...
		for keyIndex, addr := range m.addrs {
			...
			m.utxos[op] = &utxo{
				value:          navutil.Amount(output.Value),
				keyIndex:       keyIndex,
...
func (m *memWallet) newAddress() (navutil.Address, error) {
	index := m.hdIndex
	...
	err = m.rpc.LoadTxFilter(false, []navutil.Address{addr}, nil)
	if err != nil {
		return nil, err
	}
...
// fundTx attempts to fund a transaction sending amt navcoin. The coins are
...
func (m *memWallet) fundTx(tx *wire.MsgTx, amt navutil.Amount, feeRate navutil.Amount) error {
	const (
	...
	for outPoint, utxo := range m.utxos {
		...
		reqFee := navutil.Amount(txSize * int(feeRate))
...
//
// This function is safe for concurrent access.
func (m *memWallet) CreateTransaction(outputs []*wire.TxOut, feeRate navutil.Amount) (*wire.MsgTx, error) {
	m.Lock()
	defer m.Unlock()
	...
	var outputAmt navutil.Amount
	for _, output := range outputs {
		outputAmt += navutil.Amount(output.Value)
		tx.AddTxOut(output)
	}
...
//
// This function is safe for concurrent access.
func (m *memWallet) ConfirmedBalance() navutil.Amount {
	m.RLock()
	defer m.RUnlock()
