...
// Harness fully encapsulates an active navd process to provide a unified
// platform for creating rpc driven integration tests involving navd. The
// active navd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active navd process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*navutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the navd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []navutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure navd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created navd
...
func (h *Harness) NewAddress() (navutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() navutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate navutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate navutil.Amount) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*navutil.Tx, blockVersion int32,
	blockTime time.Time) (*navutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*navutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*navutil.Block, error) {
	...
	prevBlock := navutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "navd", "rpctest")
